...
function FilterPanel() {
    ...
    const activeFilterCount =
        selectedBrands.length + selectedSubfamilies.length;
    ...
    if (filtersLoading) {
        return (
            <div className="flex items-center gap-2 px-4 py-2 mb-4 text-sm font-semibold text-white rounded-md bg-accent-400">
    ...
    if (error) {
        return (
            <div className="text-sm text-red-600">Error loading filters</div>
        );
    }
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                Filters
                {activeFilterCount > 0 && (
                    <span className="flex items-center justify-center w-5 h-5 text-xs font-bold text-white rounded-full bg-accent-700">
                        {activeFilterCount}
                    </span>
                )}
            </button>
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-auto">
                        <div className="w-[90%] sm:w-[420px] bg-white rounded-xl shadow-md p-6 animate-fadeIn relative max-h-[80vh] overflow-y-auto">
                            ...
                            <h2 className="mb-4 text-xl font-bold text-primary-600">
                                Filters
                            </h2>
                            ...
                            <div className="mb-4">
                                <button
                                    className="flex items-center justify-between w-full pb-2 font-medium text-gray-700 transition border-b hover:text-primary-500"
                                            ...
                                            filters.brands.map((brand) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        className="w-4 h-4 cursor-pointer text-primary-500 focus:ring-primary-400"
                                                    />
                                                    <label
                                                        htmlFor={`brand-${brand.id}`}
                                                        className="flex-1 text-gray-600 cursor-pointer hover:text-primary-600"
                                                    >
                                                        {brand.name}
                                                    </label>
                                                </li>
                            ...
                            <div className="mb-4">
                                <button
                                    className="flex items-center justify-between w-full pb-2 font-medium text-gray-700 transition border-b hover:text-primary-500"
                                    onClick={() =>
                                        setIsSubfamilyOpen(!isSubfamilyOpen)
                                    }
                            ...
                            <div className="flex justify-between gap-3 mt-6">
                                ...
                                <button
                                    className="flex-1 px-4 py-2 text-sm font-semibold text-white transition rounded-full bg-accent-400 hover:bg-accent-500"
